...
public class APlayerActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public static void playVideo(Context context, String title, String url, boolean isLive, int position) {
        Intent intent = new Intent(context, APlayerActivity.class);
    ...
    public static void playVideo(Context context, String title, String url, boolean isLive, int position,int requestCode ) {
        Intent intent = new Intent(context, APlayerActivity.class);
        intent.putExtra("title", title);
        intent.putExtra("url", url);
        intent.putExtra("isLive", isLive);
    ...
    private void initPlayConfig(View displayView) {
        ...
        mPlayConfig = new PlayConfig(new ConfigVideo(aPlayer, url, playerListener, displayView),
    ...
    private void initAPlayer() {
        ...
        TextView tvFrames = findViewById(R.id.tvFrames);
        tvFrames.setOnClickListener(this);
        ...
        aPlayer.setOnPlayStateChangeListener((i, i1) -> {
            ...
            if (i == APlayerAndroid.PlayerState.APLAYER_PLAYING) {
                ...
                startUIUpdateThread();
                play_large.setVisibility(View.GONE);
            } else {
                play_large.setVisibility(View.VISIBLE);
    ...
    private static float seekBarePosToPlaySpeed(float pos)
    {
        float exponent = pos - 6;
        final int BASE = 2;
    ...
    private void setPlaySpeed(){
        //0 ~ 13  6 是默认
        final String[] items = { "0.5X","1.0X","1.25X","1.5X","2.0X" };
        PlayerMenuDialog dialog = PlayerMenuDialog.showDialog(this,Arrays.asList(items),(dialog2,i)->{
            speedIndex = i;
            if(mPlayConfig!=null){
                switch (i){
                    case 0:
                        mPlayConfig.getConfigVideo().setPlaySpeed(seekBarePosToPlaySpeed(3));
                        break;
                    case 1:
                        mPlayConfig.getConfigVideo().setPlaySpeed(seekBarePosToPlaySpeed(6));
                        break;
                    case 2:
                        mPlayConfig.getConfigVideo().setPlaySpeed(seekBarePosToPlaySpeed(7.5f));
                        break;
                    case 3:
                        mPlayConfig.getConfigVideo().setPlaySpeed(seekBarePosToPlaySpeed(9));
                        break;
                    case 4:
                        mPlayConfig.getConfigVideo().setPlaySpeed(seekBarePosToPlaySpeed(12));
                        break;
                }
            }
            dialog2.dismiss();
        });
        dialog.setSelectPosition(speedIndex);

    }

    private void setFrames(){
        final String[] items = { "默认","填充","16:9","4:3", };
        PlayerMenuDialog dialog = PlayerMenuDialog.showDialog(this,Arrays.asList(items),(dialog2,i)->{
            framesIndex = i;
            if(mPlayConfig!=null){
                switch (i){
                    case 0:
                        mPlayConfig.getConfigVideo().setAspectRatioType(IConfigVideo.AspectRatioType.RatioNative,"");
                        break;
                    case 1:
                        mPlayConfig.getConfigVideo().setAspectRatioType(IConfigVideo.AspectRatioType.RationFullScreen,"");
                        break;
                    case 2:
                        mPlayConfig.getConfigVideo().setAspectRatioType(IConfigVideo.AspectRatioType.RatioCustom,"16;9");
                        break;
                    case 3:
                        mPlayConfig.getConfigVideo().setAspectRatioType(IConfigVideo.AspectRatioType.RatioCustom,"4;3");
                        break;
                }
            }
            dialog2.dismiss();
        });
        dialog.setSelectPosition(framesIndex);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.v_play) {
            ...
            this.aPlayer.play();
        }else if (v.getId() == R.id.tvSpeed){
            setPlaySpeed();
        }else if (v.getId() == R.id.tvFrames) {
            setFrames();
        }else if (v.getId() == R.id.img_more) {
            screenCasting();
        }
    }
...
/*
* cling-core-2.1.2.jar

cling-support-2.1.2.jar
* */
    }
    ...
    // 在类成员变量中新增：
    private boolean isLongPressSpeedActive = false;
    private Runnable longPressSpeedRunnable;
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        switch (motionEvent.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                this.movedY = y;
                // 启动长按检测任务：500ms后设置2.0倍速，并显示提示
                longPressSpeedRunnable = new Runnable() {
                    @Override
                    public void run() {
                        if (action == 1 && mPlayConfig != null) {
                            isLongPressSpeedActive = true;
                            mPlayConfig.getConfigVideo().setPlaySpeed(seekBarePosToPlaySpeed(12)); // 2.0X速度
                            tv_info.setVisibility(VISIBLE);
                            tv_info.setText("x2.0 加速中");
                        }
                    }
                };
                longPressHandler.postDelayed(longPressSpeedRunnable, 500);
                break;
            case MotionEvent.ACTION_MOVE:
                if (!locker) {
                    ...
                    if (this.action == 1) {
                        ...
                        if (f < 50.0f && abs > 50.0f && ((double) this.pressedX) < ((double) this.width) * 0.50d) {
                            this.action = 3;
                        }
                        if (f < 50.0f && abs > 50.0f && ((double) this.pressedX) > ((double) this.width) * 0.50d) {
                            this.action = 4;
                        }
                        // 如果手指移动导致 action 发生变化，则取消长按任务
                        if (this.action != 1) {
                            longPressHandler.removeCallbacks(longPressSpeedRunnable);
                        }
                    }
                    switch (this.action) {
                        case 2:
            ...
            case MotionEvent.ACTION_UP:
                // 取消延时任务
                longPressHandler.removeCallbacks(longPressSpeedRunnable);
                // 如果处于长按调速状态，则恢复1.0倍速，并隐藏提示
                if (isLongPressSpeedActive && mPlayConfig != null) {
                    mPlayConfig.getConfigVideo().setPlaySpeed(seekBarePosToPlaySpeed(6)); // 恢复1.0X速度
                    isLongPressSpeedActive = false;
                    // 隐藏提示，可以在后续的延时任务中隐藏tv_info
                    tv_info.setVisibility(GONE);
                }
                if (System.currentTimeMillis() - runtime > 300) {
    ...
    protected void hideBottomUIMenu() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            window.getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION);

            // 设置状态栏颜色为透明
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            window.setStatusBarColor(Color.BLACK);
        } else {
            // 对于低于Lollipop的版本，使用透明状态栏
...
//        decorView.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_STABLE | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION);
    ...
    protected void onDestroy() {
        if (playerStatusListener != null) {
            playerStatusListener.onDestroy();
        }
